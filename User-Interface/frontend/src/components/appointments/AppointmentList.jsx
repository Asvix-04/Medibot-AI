    ...
    <div className="space-y-8">
      {Object.entries(groupedAppointments).map(([date, dateAppointments]) => (
        <div key={date} className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border shadow-sm`}>
          <h3 className={`text-lg font-medium mb-4 pb-2 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            {formatDate(date)}
          </h3>
                  ...
                  <div>
                    <button
                      ...
                      className={`text-sm px-3 py-1 rounded-lg ${
                        darkMode ? 'bg-red-900/30 text-red-200 hover:bg-red-900/50' : 'bg-red-100 text-red-700 hover:bg-red-200'
                      }`}
                    >
                      Cancel
                    </button>
