...
const Features = () => {
  ...
  return (
    <section id="features" className="w-full max-w-7xl mx-auto px-6 py-20 relative" style={{ opacity: 1 }}>
          ...
          {features.map((feature, index) => (
            ...
            >
              <div className='tracking-tight flex items-center space-x-3 text-white text-xl font-semibold'>
                <div className="p-2 bg-purple-500/10 rounded-lg" style={{ color: '#6366f1' }}>
                  {feature.icon}
                </div>
                <span>{feature.title}</span>
              </div>
              <p className='p-6 pt-0 text-slate-300 text-base'>{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
