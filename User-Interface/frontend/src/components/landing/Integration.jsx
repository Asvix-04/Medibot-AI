...
const Integration = () => {
 ...
 return (
  <section className="py-20 relative overflow-hidden w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ opacity: 1 }}>
    <div className="relative z-10">
      <div className="text-center md:text-left">
        <h2 className="text-3xl font-extrabold sm:text-4xl text-white">
          Seamless Integrations
        </h2>
        <p className="mt-4 max-w-2xl text-xl mx-auto md:mx-0" style={{ color: '#d6d4d4' }}>
          Medibot works with your existing health technology ecosystem.
        </p>
      </div>

      <div className="mt-16">
        <div className="relative rounded-xl shadow-xl p-6 sm:p-8 md:p-12 overflow-hidden w-full bg-slate-800/50 backdrop-blur-sm border border-white/10 shadow-purple-500/5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {integrations.map((integration, index) => (
              <div key={index} className="flex items-center space-x-4" data-aos="fade-up" data-aos-delay={integration.delay}>
                <div className="flex-shrink-0 h-12 w-12 rounded-full bg-gradient-to-br from-purple-500/20 to-blue-500/20 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <span style={{ color: '#d6d4d4' }} className="font-medium text-sm sm:text-base">{integration.name}</span>
              </div>
            ))}
          </div>

          <div className="mt-10 text-center">
            <p style={{ color: '#d6d4d4', opacity: 0.8 }} className="mb-6 text-sm sm:text-base">
      ...
      <svg width="404" height="404" fill="none" viewBox="0 0 404 404">
        <defs>
          <pattern id="integration-pattern" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
            <rect width="4" height="4" fill="currentColor" style={{ color: '#6366f1', opacity: 0.1 }} />
          </pattern>
        </defs>
